function [objectcounts,C]=corrfishcountquantification(img1,img2)

img1=double(img1);
img2=double(img2);

pixelsize=0.13;  
whitenoise='y';  %for simulation 'n' and for experiments should be 'y'
...
%Calculate the number of dots in autocorr of the first image
...
% transciprtsperPSFarea=(G12Amplitude/(G11*G22))*((pi*mean(Auto2fit(:,2))*mean(Auto2fit(:,3)))/(pi*mean(Auto1fit(:,2))*mean(Auto1fit(:,3))));
